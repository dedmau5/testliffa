...
import losAngeles from '../test-data/los-angeles';
import { Translate } from '../../../../tools';

describe('Independent-flow - city - flight list', () => {
  before(() => {
    const environmentUrl = Translate(flight[browser.options.tc.environment]);

    const losAngelesQuery = query.flightList
      .replace('{QueryDepID}', losAngeles.departureId)
      .replace('{QueryResID}', losAngeles.resortId)
      .replace('{QueryDepDate}', losAngeles.departureDate)
      .replace('{QueryRetDate}', losAngeles.returnDate)
      .replace('{QueryDur}', losAngeles.duration)
      .replace('{QueryRoomAges}', losAngeles.roomAges)
      .replace('{QueryUnits}', losAngeles.units);

    Page.goTo(environmentUrl + losAngelesQuery);
  });
