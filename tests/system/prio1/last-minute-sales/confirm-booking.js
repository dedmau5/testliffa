import CharterBooking from '../../../../pages/apps/last-minute-sales/last-minute-sales-pageobject';
import { CharterTranslations } from '../../../../localization/booking/charter/index';
...
export function ConfirmBooking(sharedData) {
    describe('Can Confirm the Booking', function () {
        ...
        it( "Total Price equals the Price seen under My Choice", function () {
            ...
            expect( CharterBooking.steps.confirmBooking.totalPrice ).to.equal( CharterBooking.myChoices.travelInformation.price,
                "Price mismatch!"
            );
        });
        ...
        it( "Price equals the Price seen on the previous page", function () {
            ...
            expect( CharterBooking.steps.confirmBooking.totalPrice ).to.equal( sharedData.extrasTotalPrice,
        ...
        it( `Can accept Ving's travel terms`, function () {
            ...
            expect( CharterBooking.steps.confirmBooking.checkboxes.terms.isChecked() ).to.equal( false,
                "The travel terms checkbox shouldn't have been checked!"
            );

            CharterBooking.steps.confirmBooking.checkboxes.terms.click();

            expect( CharterBooking.steps.confirmBooking.checkboxes.terms.isChecked() ).to.equal( true,
                "The travel terms checkbox should have been checked!"
            );
        });

        it( `Can confirm the booking within ${timeout/1000} s using the confirm button`, function () {
            ...
            if ( headingLabel === CharterTranslations.steps.confirmedBooking.heading) {
                const bookingNumber = CharterBooking.steps.confirmedBooking.bookingNumber;
                console.warn(`\nBooking number: ${bookingNumber}`);
                browser.options.tc.bookingNumber = bookingNumber;
            }
        });

        // TODO: Skriv ut vilken form av bokning den fick. PBC, riktig bokning etc.
    });
}