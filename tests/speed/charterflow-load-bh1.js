...
describe('Load test for charterflow with Bookinghub 1', () => {
  before(() => {
    console.log(`Will serially load ${lines.length - 1} different URL:s`);
    StartPage.open();
    browser.sessionStorage('POST', { key: 'DT_TESTNAME', value: 'TEAM1: Charterflow bh1 load test' });
    browser.sessionStorage('POST', { key: 'DT_TESTRUNID', value: 'CharterFlow-bh1-load' });
  });
  ...
  describe(`Carry out every search carried out by customers in production (${lines.length - 1} URL:s)`, () => {
    it('Surfing to each URL and wait for page to load', () => {
      for (let x = 4; x < lines.length - 1; x += 1) {
        ...
        parsedUrl = `https://www.ving.se${lineSplit[0]}?${lineSplit[1]}`;
