...
const user = { name: 'martin.wrangenby@thomascook.se', pw: 'Test1234' };
...
describe('Tests to load new my page', () => {
  it('Opening my page', () => {
    StartPage.open();
    browser.url('/login');
    ...
    for (let x = 0; x < iterations; x += 1) {
      try {
        ...
        browser.sessionStorage('POST', { key: 'DT_TESTNAME', value: 'TEAM1: Speed test for new My page with multiple bookings' });
        browser.sessionStorage('POST', { key: 'DT_TESTRUNID', value: 'speed_mypage_new_multiple_trips' });
        browser.url('https://www.ving.se/minsida2');
        browser.waitForVisible('#bookinglist-page', timeout);
